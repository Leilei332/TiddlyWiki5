...
class WikiParser {
	...
	*/
	instantiateRules(classes, type, startPos) {
		var rulesInfo = [], self = this;
	...
	*/
	findNextMatch(rules, startPos) {
		// Find the best matching rule by finding the closest match position
		var matchingRule, matchingRulePos = this.sourceLength;
	...
	*/
	parseBlock(terminatorRegExpString) {
	...
	*/
	parseBlocks(terminatorRegExpString) {
	...
	*/
	parseBlocksTerminated(terminatorRegExpString) {
	...
	*/
	parseBlocksTerminatedExtended(terminatorRegExpString) {
		var terminatorRegExp = new RegExp(terminatorRegExpString, "mg"), result = {
			tree: []
		};
		// Skip any whitespace
		this.skipWhitespace();
	...
	*/
	parseInlineRun(terminatorRegExp, options) {
		if(terminatorRegExp) {
	...
	parseInlineRunUnterminated(options) {
		var tree = [];
	...
	parseInlineRunTerminated(terminatorRegExp, options) {
		var ex = this.parseInlineRunTerminatedExtended(terminatorRegExp, options);
		return ex.tree;
	}

	parseInlineRunTerminatedExtended(terminatorRegExp, options) {
		options = options || {};
		var tree = [];
	...
	*/
	pushTextWidget(array, text, start, end) {
	...
	*/
	parseClasses() {
		var classRegExp = /\.([^\s\.]+)/mg, classNames = [];
